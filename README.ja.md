...
Codexで、ゲームのフィールド/探索BGMを編集可能なマルチトラックMIDIとして作るためのSkillです。

自然言語で依頼すると、Codexが `desert field`、`snowfield`、`tropical coast` のようなフィールド場面を、作曲ブリーフ、ループ設計、再生可能な `.mid` ファイルへ落とし込みます。出力したMIDIは Midiano、MuseScore、LMMS、DAW、ゲーム制作パイプラインで扱えます。

v0.1 は実験版です。現時点で公開デモとして確認している主な強みは、フィールド/探索BGMのMIDIプロトタイプです。戦闘BGMは初期テストを1本追加しています。
...
`compose-game-music` は Codex-first の音楽制作ワークフローです。ゲームのフィールド場面を、プレイヤーの移動、場所の印象、ループ疲労、モチーフ、低音、和声、レイヤー構成、MIDI出力へ変換します。

- **MIDI-first**: 編集、確認、再アレンジ、DAW連携がしやすい。
- **Field-scene aware**: 雰囲気だけでなく、場所とプレイヤーの移動を扱う。
...
v0.1で強く見せられるのは、フィールド/探索BGMのMIDIプロトタイプです。戦闘BGMは初期テストを1本追加しています。

戦闘BGM、ボス曲、ダンジョン、街、アダプティブミュージック、SFX設計のワークフローはSkill内にあります。ただし、公開デモとして十分に検証できているのはまだ `砂漠 / 雪国 / 南国` の3つです。
...
- **v0.1**: Skill本体、出力契約、フィールド/探索MIDIデモ。
- **v0.2**: MIDI生成スクリプトとフィールドプリセットの強化。
- **v0.3**: 戦闘、ボス、ダンジョン、街、メニューのMIDI例。
